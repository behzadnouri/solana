...
pub mod serde_compact_vote_state {
    use {
        super::*,
        crate::{
    ...
    struct CompactVoteStateUpdate {
        ...
        compact_lockouts: Vec<CompactLockout>,
        hash: Hash,
    ...
    where
        S: Serializer,
    {
        let compact_lockouts = vote_state_update.lockouts.iter().scan(
            ...
            |slot, lockout| {
                ...
                let compact_lockout = CompactLockout {
                    offset,
                    confirmation_count: lockout.confirmation_count,
                };
                *slot = lockout.slot;
                Some(Ok(compact_lockout))
            },
        );
        let compact_vote_state_update = CompactVoteStateUpdate {
            root: vote_state_update.root.unwrap_or(Slot::MAX),
            compact_lockouts: compact_lockouts.collect::<Result<_, _>>()?,
        ...
        D: Deserializer<'de>,
    {
        ...
        let lockouts =
            compact_lockouts
                .iter()
                .scan(root.unwrap_or_default(), |slot, compact_lockout| {
                    *slot = match slot.checked_add(compact_lockout.offset) {
                        None => {
                    ...
                    let lockout = Lockout {
                        slot: *slot,
                        confirmation_count: compact_lockout.confirmation_count,
                    };
                    Some(Ok(lockout))
                });
        Ok(VoteStateUpdate {
