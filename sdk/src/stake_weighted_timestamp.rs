      ...
      V: Borrow<(Slot, UnixTimestamp)>,
{
    let (stake_weighted_timestamps_sum, total_stake) = unique_timestamps
        .into_iter()
        .filter_map(|(vote_pubkey, slot_and_timestamp)| {
            let (timestamp_slot, timestamp) = slot_and_timestamp.borrow();
...
fn calculate_bounded_stake_weighted_timestamp<T, I, K, V>(
    unique_timestamps: I,
    stakes: &HashMap<Pubkey, (u64, T /*Account|ArcVoteAccount*/)>,
    slot: Slot,
    slot_duration: Duration,
      ...
      V: Borrow<(Slot, UnixTimestamp)>,
{
    ...
    for (vote_pubkey, slot_and_timestamp) in unique_timestamps {
        let (timestamp_slot, timestamp) = slot_and_timestamp.borrow();
