...
fn recv_mmsg(socket: &std::net::UdpSocket, packets: &mut [Packet]) -> io::Result<(usize, usize)> {
    let mut i = 0;
...
impl UdpSocket<'_> {
    #[cfg(not(target_os = "linux"))]
    pub fn recv_mmsg(&mut self, packets: &mut [Packet]) -> io::Result<(usize, usize)> {
        recv_mmsg(self.socket, packets)
    }
...
#[cfg(test)]
mod tests {
    use crate::packet::{Packet, PACKET_DATA_SIZE};
    ...
    pub fn test_recv_mmsg_one_iter() {
        ...
        let mut reader = super::UdpSocket::from(&reader);
    ...
    pub fn test_recv_mmsg_multi_iter_timeout() {
        ...
        let _recv = reader.recv_mmsg(&mut packets[..]);
