...
fn prepare_if_nonce_account<'a>(
    ...
    &(durable_nonce, separate_domain): &(DurableNonce, bool),
    lamports_per_signature: u64,
) -> bool {
    if let Some((nonce, rollback)) = maybe_nonce {
        if address == nonce.address() {
            ...
            if let NonceState::Initialized(ref data) = state {
                ...
                let nonce_versions = NonceVersions::new_current(nonce_state, separate_domain);
...
mod tests {
    ...
    fn create_accounts_prepare_if_nonce_account() -> (
        ...
        let data = NonceVersions::new_current(
            NonceState::Initialized(nonce::state::Data::default()),
            /*separate_domains:*/ true,
        );
    ...
    fn test_prepare_if_nonce_account_expected() {
        ...
        expect_account
            .set_state(&NonceVersions::new_current(
    ...
    fn test_nonced_failure_accounts_rollback_from_pays() {
        ...
        let nonce_state = NonceVersions::new_current(
            NonceState::Initialized(nonce::state::Data::new(
