    ...
    pub fn get_complete_block(
        ...
        if slot_meta.is_full() {
            ...
            if !slot_entries.is_empty() {
                ...
                let parent_slot_entries = match slot_meta.parent_slot {
                    None => Vec::default(),
                    Some(slot) => self
                ...
                let block = ConfirmedBlock {
                    ...
                    parent_slot: slot_meta.parent_slot.unwrap_or(Slot::MAX),
                    transactions: self
...
fn handle_chaining_for_slot(
...

    {
        ...
        if slot != 0 {
            // TODO this is re-implementing old logic but does not seem right.
            let prev_slot = meta_mut.parent_slot.unwrap_or(Slot::MAX);
