...
use {
    crate::{
        shred::{
            ...
            dispatch,
            merkle::{
                get_merkle_node, get_merkle_proof, get_merkle_root, MerkleProofEntry,
                SIZE_OF_MERKLE_PROOF_ENTRY, SIZE_OF_MERKLE_ROOT,
            },
...
//      | merkle root of previous FEC set | merkle proof
...
impl Shred {
    ...
    fn from_payload(shred: Vec<u8>) -> Result<Self, Error> {
        match shred::layout::get_shred_variant(&shred)? {
            ...
            | ShredVariant::MerkleData(_) => Err(Error::InvalidShredVariant),
            ShredVariant::ChainedCode(_) => Ok(Self::ShredCode(ShredCode::from_payload(shred)?)),
            ShredVariant::ChainedData(_) => Ok(Self::ShredData(ShredData::from_payload(shred)?)),
        }
    }
}
...
impl ShredData {
    ...
    fn proof_size(&self) -> Result<u8, Error> {
        match self.common_header.shred_variant {
            ShredVariant::ChainedData(proof_size) => Ok(proof_size),
    ...
    fn proof_offset(proof_size: u8) -> Result<usize, Error> {
        Ok(Self::SIZE_OF_HEADERS + Self::capacity(proof_size)? + SIZE_OF_MERKLE_ROOT)
    }
    ...
    fn from_recovered_shard(signature: &Signature, mut shard: Vec<u8>) -> Result<Self, Error> {
        ...
        let ShredVariant::ChainedData(proof_size) = common_header.shred_variant else {
    ...
    pub(super) fn get_merkle_root(shred: &[u8], proof_size: u8) -> Option<Hash> {
        debug_assert_eq!(
            ...
            ShredVariant::ChainedData(proof_size)
        );
...
impl ShredCode {
    ...
    fn proof_size(&self) -> Result<u8, Error> {
        match self.common_header.shred_variant {
            ShredVariant::ChainedCode(proof_size) => Ok(proof_size),
            _ => Err(Error::InvalidShredVariant),
        }
    }
    ...
    fn from_recovered_shard(
        ...
        let ShredVariant::ChainedCode(proof_size) = common_header.shred_variant else {
            return Err(Error::InvalidShredVariant);
        };
        let shard_size = shard.len();
    ...
    pub(super) fn get_merkle_root(shred: &[u8], proof_size: u8) -> Option<Hash> {
        debug_assert_eq!(
            shred::layout::get_shred_variant(shred).unwrap(),
            ShredVariant::ChainedCode(proof_size)
        );
        // Shred index in the erasure batch.
        let index = {
...
impl<'a> ShredTrait<'a> for ShredData {
    ...
    fn from_payload(mut payload: Vec<u8>) -> Result<Self, Error> {
        ...
        let common_header: ShredCommonHeader = deserialize_from_with_limit(&mut cursor)?;
        if !matches!(common_header.shred_variant, ShredVariant::ChainedData(_)) {
            return Err(Error::InvalidShredVariant);
        }
        let data_header = deserialize_from_with_limit(&mut cursor)?;
        let shred = Self {
    ...
    fn sanitize(&self) -> Result<(), Error> {
        ...
        if !matches!(shred_variant, ShredVariant::ChainedData(_)) {
            return Err(Error::InvalidShredVariant);
        }
...
impl<'a> ShredTrait<'a> for ShredCode {
    ...
    fn from_payload(mut payload: Vec<u8>) -> Result<Self, Error> {
        let mut cursor = Cursor::new(&payload[..]);
        let common_header: ShredCommonHeader = deserialize_from_with_limit(&mut cursor)?;
        if !matches!(common_header.shred_variant, ShredVariant::ChainedCode(_)) {
            return Err(Error::InvalidShredVariant);
        }
    ...
    fn sanitize(&self) -> Result<(), Error> {
        let shred_variant = self.common_header.shred_variant;
        if !matches!(shred_variant, ShredVariant::ChainedCode(_)) {
            return Err(Error::InvalidShredVariant);
        }
        let _ = self.merkle_proof()?;
