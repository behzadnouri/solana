...
impl<T: 'static> ClusterNodesCache<T> {
    pub(crate) fn get(
        ...
        // but also expired.
        let get_epoch_entry = |cache: &LruCache<Epoch, _>, epoch: Epoch| {
            ...
            (asof.elapsed() < self.ttl).then(|| entry.clone())
        };
        ...
        let entry: Arc<OnceLock<_>> = entry.unwrap_or_else(|| {
            ...
            get_epoch_entry(&cache, epoch).unwrap_or_else(|| {
        ...
        let (_, nodes) = entry.get_or_init(|| {
            ...
            let epoch_staked_nodes = [root_bank, working_bank]
                ...
                .unwrap_or_else(|| {
                    error!("ClusterNodesCache::get: unknown Bank::epoch_staked_nodes for epoch: {epoch}.");
