...
impl CrdsGossipPush {
    ...
    fn push_options(
        ...
        gossip_validators: Option<&HashSet<Pubkey>>,
        socket_addr_space: &SocketAddrSpace,
    ) -> Vec<(/*weight:*/ u64, /*node:*/ LegacyContactInfo)> {
        let now = timestamp();
        let mut rng = rand::thread_rng();
        ...
        crds.get_nodes()
            ...
            .filter(|info| {
                info.id != *self_id
                    && LegacyContactInfo::is_valid_address(&info.gossip, socket_addr_space)
...
mod tests {
    ...
    fn test_process_push_old_version() {
        let crds = RwLock::<Crds>::default();
        let push = CrdsGossipPush::default();
        let mut ci = LegacyContactInfo::new_localhost(&solana_sdk::pubkey::new_rand(), 0);
        ci.wallclock = 1;
    ...
    fn test_refresh_active_set() {
        ...
        let value1 = LegacyContactInfo::new_localhost(&solana_sdk::pubkey::new_rand(), 0);
        ...
        let mut value2 = LegacyContactInfo::new_localhost(&solana_sdk::pubkey::new_rand(), 0);
    ...
    fn test_active_set_refresh_with_bank() {
        ...
        for i in 1..=100 {
            let peer = CrdsValue::new_unsigned(CrdsData::LegacyContactInfo(
                LegacyContactInfo::new_localhost(&solana_sdk::pubkey::new_rand(), time),
            ));
    ...
    fn test_new_push_messages() {
        ...
        let push = CrdsGossipPush::default();
        let mut ping_cache = new_ping_cache();
        let peer = LegacyContactInfo::new_localhost(&solana_sdk::pubkey::new_rand(), 0);
    ...
    fn test_personalized_push_messages() {
        ...
        let peers: Vec<_> = vec![0, 0, now]
            .into_iter()
            .map(|wallclock| {
                let mut peer = LegacyContactInfo::new_rand(&mut rng, /*pubkey=*/ None);
