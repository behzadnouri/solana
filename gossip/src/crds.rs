...
impl Crds {
    ...
    fn error_crds_data_size(data: &CrdsData) {
        use rand::Rng;
        if !rand::thread_rng().gen_ratio(1, 1000) {
            return;
        }
        let name: &str = match data {
        ...
        error!("crds_data_size: {name}, {}", bincode::serialized_size(data).unwrap());
    }
    ...
    pub fn insert(
        ...
        match self.table.entry(label) {
            Entry::Vacant(entry) => {
                Self::error_crds_data_size(&value.value.data);
