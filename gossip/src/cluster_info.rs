...
impl ClusterInfo {
    ...
    pub fn push_epoch_slots(&self, mut update: &[Slot]) {
        ...
        let current_slots: Vec<_> = {
            let _st = ScopedTimer::from(&self.stats.epoch_slots_lookup);
    ...
    pub fn push_vote(&self, tower: &[Slot], vote: Transaction) {
        ...
        let vote_index = {
            let _st = ScopedTimer::from(&self.stats.push_vote_read);
            (0..MAX_LOCKOUT_HISTORY as u8)
                .filter_map(|ix| {
                    let vote = CrdsValueLabel::Vote(ix, self_pubkey);
                    let vote = self.gossip.crds.get::<&CrdsData>(&vote)?;
                    num_crds_votes += 1;
                    match *vote {
    ...
    pub fn get_votes(&self, cursor: &mut Cursor) -> Vec<Transaction> {
        let _st = ScopedTimer::from(&self.stats.get_votes);
        let votes = self.gossip.crds.get_votes(cursor);
        let txs: Vec<Transaction> = votes
            .map(|vote| {
    ...
    {
        let _st = ScopedTimer::from(&self.stats.get_accounts_hash);
        let key = CrdsValueLabel::AccountsHashes(*pubkey);
        let value = self.gossip.crds.get::<&CrdsValue>(&key)?;
        Some(map(&value.accounts_hash().unwrap().hashes))
    }
    ...
    pub fn all_tvu_peers(&self) -> Vec<ContactInfo> {
        let self_pubkey = self.id();
        let _st = ScopedTimer::from(&self.stats.all_tvu_peers);
        self.gossip
            .crds
            .get_nodes_contact_info()
            .filter(|x| {
    ...
    fn append_entrypoint_to_pulls(
        ...
        let entrypoint = {
            ...
            if !pulls.is_empty() {
                ...
                let _st = ScopedTimer::from(&self.stats.entrypoint);
                if self
                    .gossip
                    .crds
                    .get_nodes_contact_info()
    ...
    fn new_push_requests(&self, stakes: &HashMap<Pubkey, u64>) -> Vec<(SocketAddr, Protocol)> {
        ...
        let push_messages: Vec<_> = {
            let _st = ScopedTimer::from(&self.stats.new_push_requests2);
            push_messages
                .into_iter()
