...
impl ClusterInfo {
    ...
    fn handle_batch_pull_requests(
        ...
        outliers: &mut HashMap<(SocketAddr, ContactInfo), usize>,
    ) {
        ...
        if !requests.is_empty() {
            ...
            } in &requests
            {
                ...
                if k < 10 {
                    let caller = caller.contact_info().cloned().unwrap();
                    let key = (*from_addr, caller);
                    *outliers.entry(key).or_default() += 1;
                }
            }
