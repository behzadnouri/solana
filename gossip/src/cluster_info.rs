...
impl ClusterInfo {
    ...
    fn run_socket_consume(
        ...
        let mut num_packets = packets.len();
        ...
        for packet_batch in receiver.try_iter() {
            ...
            while num_packets > MAX_GOSSIP_TRAFFIC {
                ...
                let Some(num_packets_dropped) = packets.pop_front().as_ref().map(PacketBatch::len)
                else {
                    break;
                };
                num_packets -= num_packets_dropped;
                self.stats
                    .gossip_packets_dropped_count
                    .add_relaxed(num_packets_dropped as u64);
            }
        }
